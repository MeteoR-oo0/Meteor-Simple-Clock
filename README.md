...
- デジタル時計（時・分、点滅するコロン、12/24H切り替え対応） 
- 日付表示（複数フォーマット切替、非表示も可） 
- CPU / GPU / 冷却水温のリアルタイム表示（設定で非表示対応） 
- 背景画像・背景色の切り替え（透過・ぼかし対応） 
- フォント切り替え　計9種 + フォントウェイト選択 
- 秒数に連動するシークバー（秒針、非表示可） 
- コロン（：）の表示を四角・丸に切り替え可能 
- 文字色のカスタマイズ 
- テキストシャドウ（ON/OFF） 
- 各種設定を保存・即時反映（Kraken実機とも同期） 
- 設定初期化ボタンつき 
...
### v1.3.1（2025-09-17）
- 日付のフォント・長さによって2行に表示される場合がありましたので、必ず1行になるように修正
- CPU / GPU / LIQの非表示をそれぞれ個別に非表示できるように対応 

### v1.3.0 – 日付フォーマット切替 & 秒針/区切り線制御など（2025-09-09）
- フォントを追加＆太字に任意に変更できるように
- 日付フォーマットを複数選択可能に（例：`M/D ddd`、`D.M ddd`、`ddd, MMM D` など）
- 日付の **非表示** 設定を追加
- 秒数バー（シークバー）と区切り線をまとめて非表示にできるオプションを追加
- コロン（：）の形を **四角/丸** で切り替え可能に
- 文字スケール機能を調整（縮小表示にすれば、Kraken Plusなどでも表記崩れが起きなくなります）

### v1.2.0 – 12/24H表示切替対応など（2025-08-12） 
- 時計の表記を 12/24H で切り替えができるように対応 
- CPU / GPU / 冷却水温の表記を非表示できるように対応 
- 設定画面のUIを調節 
- 解像度の設定を一時的に削除（他Krakenシリーズだと正常に表示されなかったため） 
